...
//Returns the number of dirty blocks in the cache 
int Cache::getNumDirtyBlocks(){
	
	int total = 0;
	
	//Loops through every set in the cache and counts the remaining dirty blocks in each
	int i;
	for(i = 0; i < (cacheSize / (blockSize * blocksPerSet)); i++) {
		total += sets[i]->getSetNumDirtyBlocks();	
	}	
